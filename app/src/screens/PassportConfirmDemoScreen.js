...
const FAKE_PASSPORT = {
  fullName:        'YAMAMOTO HANA',
  passport_number: 'AB1234567',
  nationality:     'JPN',
  date_of_birth:   '0100215',
  sex:             'Female',
};

export default function PassportConfirmDemoScreen({ navigation }) {
  const [isEditing, setIsEditing] = useState(false);
  const [editData, setEditData] = useState({ ...FAKE_PASSPORT });
  const [savedData, setSavedData] = useState({ ...FAKE_PASSPORT });

  const handleEdit   = () => setIsEditing(true);
  const handleDone   = () => { setSavedData({ ...editData }); setIsEditing(false); };
  const handleCancel = () => { setEditData({ ...savedData }); setIsEditing(false); };
  ...
  const FIELDS = [
    ...
    { label: '생년월일', key: 'date_of_birth',    autoCapitalize: 'none' },
  ];
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.card, isEditing && styles.cardEditing]}>
            {FIELDS.map(({ label, key, autoCapitalize }) => (
      ...
      <View style={styles.bottom}>
        <AnimatedButton style={styles.button} onPress={() => navigation.navigate('FaceScanDemo')}>
...
const styles = StyleSheet.create({
  ...
  backBtn: { paddingHorizontal: 20, paddingTop: 14, paddingBottom: 10, alignSelf: 'flex-start' },
  backBtnText: { fontSize: 32, color: COLORS.textDark, lineHeight: 34 },
  headerTitle: {
    position: 'absolute', left: 0, right: 0,
    textAlign: 'center', fontSize: 17,
    fontFamily: 'Hana2-Bold', color: COLORS.textDark, zIndex: -1,
  },
  headerActions: { flexDirection: 'row', marginLeft: 'auto', gap: 4 },
  headerBtn: { paddingHorizontal: 16, paddingVertical: 12, marginLeft: 'auto' },
  editBtnText:   { fontFamily: 'Hana2-Medium', fontSize: 15, color: '#05A68B' },
  doneBtnText:   { fontFamily: 'Hana2-Bold',   fontSize: 15, color: '#05A68B' },
  cancelBtnText: { fontFamily: 'Hana2-Regular', fontSize: 15, color: COLORS.textGray },
  ...
  card: { backgroundColor: '#F8F9FA', borderRadius: 16, paddingHorizontal: 16, marginBottom: 16 },
  cardEditing: { borderWidth: 1.5, borderColor: '#05A68B' },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 14 },
  rowBorder: { borderBottomWidth: 1, borderBottomColor: '#EBEBEB' },
  rowLabel: { fontFamily: 'Hana2-Regular', fontSize: 14, color: COLORS.textGray, width: 72 },
  rowValue: { fontFamily: 'Hana2-Bold', fontSize: 15, color: COLORS.textDark, flex: 1, textAlign: 'right' },
  rowInput: {
    fontFamily: 'Hana2-Bold', fontSize: 15, color: COLORS.textDark,
    flex: 1, textAlign: 'right', paddingVertical: 2,
    borderBottomWidth: 1, borderBottomColor: '#05A68B',
  },
  sexToggle: { flexDirection: 'row', alignItems: 'center' },
  sexToggleText: { fontFamily: 'Hana2-Bold', fontSize: 15, color: '#05A68B' },
  sexCaret:      { fontFamily: 'Hana2-Regular', fontSize: 13, color: '#05A68B' },
  noticeBox: { backgroundColor: '#FFF8E6', borderRadius: 12, padding: 14 },
  noticeText: { fontFamily: 'Hana2-Regular', fontSize: 13, color: '#8A6C00', lineHeight: 20 },
  ...
  rescanText:  { fontFamily: 'Hana2-Regular', fontSize: 14, color: COLORS.textGray, textAlign: 'center' },
});
